...
function kahuna_posted_author_single() {
	...
	if ( $kahuna_meta_author ) {
		echo sprintf(
			...
			sprintf( esc_attr__( 'View all posts by %s', 'kahuna' ), wp_kses( get_the_author_meta( 'display_name', $author_id), array() ) ),
...
function kahuna_posted_date() {
	...
	if ( $kahuna_meta_date || $kahuna_meta_time ) {
		$date = ''; $time = '';
		if ( $kahuna_meta_date ) { $date = esc_html( get_the_date() ); }
		if ( $kahuna_meta_time ) { $time = esc_html( get_the_time() ); }
		?>
	...
	if ( ! empty( $featured_image[0] ) ) {
		...
		<div class="post-thumbnail-container" <?php cryout_schema_microdata( 'image' ); ?>>
			...
			<picture class="responsive-featured-image">
				<source media="(max-width: 1152px)" sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['kahuna_magazinelayout'], $options['kahuna_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'kahuna-featured-third' ) ); ?> 512w">
				<source media="(max-width: 800px)" sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['kahuna_magazinelayout'], $options['kahuna_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'kahuna-featured-half' ) ); ?> 800w">
				<?php if ( cryout_on_landingpage() ) { ?><source sizes="<?php echo esc_attr( cryout_gen_featured_sizes( $featured_width, $options['kahuna_magazinelayout'], $options['kahuna_landingpage'] ) ) ?>" srcset="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'kahuna-featured-lp' ) ); ?> <?php printf( '%sw', absint( $featured_width ) ) ?>">
				<?php } ?>
				<img alt="<?php the_title_attribute();?>" <?php cryout_schema_microdata( 'url' ); ?> src="<?php echo esc_url( cryout_get_picture_src( $fimage_id, 'kahuna-featured' ) ); ?>">
			</picture>
